...
### Spustit predikci
vysledky_vse = data.frame(strana=character(), aktualne=numeric(),
                          predikce_strata=numeric(),predikce_vzdelani=numeric(),
                          predikce_vek=numeric(),predikce_vzdelani_okr=numeric(),
                          predikce_vek_okr=numeric(), procent_secteno=numeric())
...
for (procent_secteno in seq(start, stop, by=5)) {
  ...
  ######## Multinom 7 vek okrsky  ######## 
  
  # Nacti
  
  load("ciselniky/ciselnik_vek_okrsky.RData")
  ...
  ### Join ciselnik
  data <- merge(ciselnik_vek_okr, vstup_posledni, by=c("cis_obec", "cis_okrsek"), all=T);
  indx_cluster <- grepl('comp', colnames(data))
  name_cluster <- colnames(data)[indx_cluster]
  indx_kstrana <- grepl('kstrana_', colnames(data))
  name_kstrana <- colnames(data)[indx_kstrana]
  ...
  vysledky = vysledky[, c("strana", "predikce")]
  names(vysledky) = c("strana", "predikce_vek_okr")
  
  vysledky_vek_okrsky = vysledky
  
  ######## Multinom 7 vzdelani okrsky ######## 
  
  # Nacti
  #            load("/Users/admin/Desktop/reseni_0922/testovani/ciselnik17.RData")
  load("ciselniky/ciselnik_vzdelani_okrsky.RData")
  # Nacti
  #vstup <- read.table(paste0(paste0("/Users/User/Desktop/MFF/sfg_volby/reseni_0922/testovani/secteno_upravene/secteno", as.character(procent_secteno)), ".csv"), sep=",", header = T)
  ...
  ### Join ciselnik
  data <- merge(ciselnik_vzdelani_okr, vstup_posledni, by=c("cis_obec", "cis_okrsek"), all=T);
  indx_cluster <- grepl('comp', colnames(data))
  name_cluster <- colnames(data)[indx_cluster]
  indx_kstrana <- grepl('kstrana_', colnames(data))
  name_kstrana <- colnames(data)[indx_kstrana]
  ...
  vysledky = vysledky[, c("strana", "predikce")]
  names(vysledky) = c("strana", "predikce_vzdelani_okr")
  
  vysledky_vzdelani_okrsky= vysledky
 
  ####### Merge   ####### 
  
  
  df_list <- list(vysledky_strata, vysledky_vzdelani,
                  vysledky_vek,vysledky_vzdelani_okrsky,
                  vysledky_vek_okrsky)      
  
  #merge all data frames together
  vysledky<-Reduce(function(x, y) merge(x, y, all=TRUE), df_list)  
...
for(strana in strany){
  ...
  plot <- ggplot(data=vysledky_plot[vysledky_plot$procent_secteno>=5, ])+
    ...
    geom_line(aes(x=procent_secteno/100, y=predikce_vek_okr, color="Predikce vek okrsky")) + 
    geom_line(aes(x=procent_secteno/100, y=predikce_vzdelani_okr, color="Predikce vzdelani okrsky")) + 
    ...
    #scale_color_hue(h = c(0, 215) + 15,limits=c("Aktualne", "Predikce vek","Predikce vzdelani","Predikce vek okrsky","Predikce vzdelani okrsky","Predikce volby 2017")) +
...
vysledky_souboj = vysledky_vse[vysledky_vse$strana=="kstrana_13",
                               c("procent_secteno", "predikce_strata",
                                 "predikce_vek", "predikce_vzdelani",
                                 "predikce_vek_okr", "predikce_vzdelani_okr",
                                 "aktualne")]
names(vysledky_souboj) = c("procent_secteno", "predikce_strata_spolu",
                           "predikce_vek_spolu","predikce_vzdelani_spolu",
                           "predikce_vek_okr_spolu","predikce_vzdelani_okr_spolu",
                           "aktualne_spolu")

vysledky_souboj = merge(vysledky_souboj, vysledky_vse[vysledky_vse$strana=="kstrana_20",
                                                      c("procent_secteno", "predikce_strata",
                                                        "predikce_vek", "predikce_vzdelani",
                                                        "predikce_vek_okr", "predikce_vzdelani_okr",
                                                        "aktualne")])
names(vysledky_souboj) = c("procent_secteno", "predikce_strata_spolu",
                           "predikce_vek_spolu","predikce_vzdelani_spolu",
                           "predikce_vek_okr_spolu","predikce_vzdelani_okr_spolu",
                           "aktualne_spolu","predikce_strata_ano",
                           "predikce_vek_ano","predikce_vzdelani_ano",
                           "predikce_vek_okr_ano","predikce_vzdelani_okr_ano",
                           "aktualne_ano")
...
vysledky_souboj["predikce_vek_okr"] = vysledky_souboj["predikce_vek_okr_ano"] - vysledky_souboj["predikce_vek_okr_spolu"]
vysledky_souboj["predikce_vzdelani_okr"] = vysledky_souboj["predikce_vzdelani_okr_ano"] - vysledky_souboj["predikce_vzdelani_okr_spolu"]
...
vysledky_vetsina["predikce_vek_adj"] = vysledky_vetsina$predikce_vek*(vysledky_vetsina$predikce_vek>=0.05)
vysledky_vetsina["predikce_vzdelani_adj"] = vysledky_vetsina$predikce_vzdelani*(vysledky_vetsina$predikce_vzdelani>=0.05)
vysledky_vetsina["predikce_vek_okr_adj"] = vysledky_vetsina$predikce_vek_okr*(vysledky_vetsina$predikce_vek_okr>=0.05)
vysledky_vetsina["predikce_vzdelani_okr_adj"] = vysledky_vetsina$predikce_vzdelani_okr*(vysledky_vetsina$predikce_vzdelani_okr>=0.05)
vysledky_vetsina["aktualne_adj"] = vysledky_vetsina$aktualne*(vysledky_vetsina$aktualne>=0.05)

vetsina = aggregate(cbind(predikce_strata_adj,predikce_vek_adj,predikce_vzdelani_adj,
                          predikce_vek_okr_adj,predikce_vzdelani_okr_adj, aktualne_adj) ~ koalice + procent_secteno,vysledky_vetsina,  sum)
vetsina_celkem = aggregate(cbind(predikce_strata_adj,predikce_vek_adj,predikce_vzdelani_adj,
                                 predikce_vek_okr_adj,predikce_vzdelani_okr_adj,aktualne_adj) ~ procent_secteno,vetsina,  sum)
names(vetsina_celkem) = c("procent_secteno", "predikce_strata_celkem",
                          "predikce_vek_celkem","predikce_vzdelani_celkem",
                          "predikce_vek_okr_celkem","predikce_vzdelani_okr_celkem",
                          "aktualne_celkem")
...
vetsina["pomer_predikce_vzdelani"] = vetsina["predikce_vzdelani_adj"]/vetsina["predikce_vzdelani_okr_celkem"]
vetsina["pomer_predikce_vek_okr"] = vetsina["predikce_vek_okr_adj"]/vetsina["predikce_vek_okr_celkem"]
vetsina["pomer_predikce_vzdelani_okr"] = vetsina["predikce_vzdelani_okr_adj"]/vetsina["predikce_vzdelani_okr_celkem"]
...
plot_list[["vetsina"]] <- ggplot(data=vetsina[(vetsina$koalice==1) & (vetsina$procent_secteno>=5), ])+
  ...
  geom_line(aes(x=procent_secteno/100, y=pomer_predikce_vek_okr, color="Predikce vek okrsky")) + 
  geom_line(aes(x=procent_secteno/100, y=pomer_predikce_vzdelani_okr, color="Predikce vzdelani okrsky")) + 
...
vysledky_vlada["predikce_vek_adj"] = vysledky_vlada$predikce_vek*(vysledky_vlada$predikce_vek>=0.05)
vysledky_vlada["predikce_vzdelani_adj"] = vysledky_vlada$predikce_vzdelani*(vysledky_vlada$predikce_vzdelani>=0.05)
vysledky_vlada["predikce_vek_okr_adj"] = vysledky_vlada$predikce_vek_okr*(vysledky_vlada$predikce_vek_okr>=0.05)
vysledky_vlada["predikce_vzdelani_okr_adj"] = vysledky_vlada$predikce_vzdelani_okr*(vysledky_vlada$predikce_vzdelani_okr>=0.05)
vysledky_vlada["aktualne_adj"] = vysledky_vlada$aktualne*(vysledky_vlada$aktualne>=0.05)

vlada = aggregate(cbind(predikce_strata_adj,predikce_vek_adj,predikce_vzdelani_adj,
                        predikce_vek_okr_adj,predikce_vzdelani_okr_adj,aktualne_adj) ~ koalice + procent_secteno,vysledky_vlada,  sum)
vlada_celkem = aggregate(cbind(predikce_strata_adj,predikce_vek_adj,predikce_vzdelani_adj,
                               predikce_vek_okr_adj,predikce_vzdelani_okr_adj,aktualne_adj) ~ procent_secteno,vlada,  sum)
names(vlada_celkem) = c("procent_secteno", "predikce_strata_celkem",
                        "predikce_vek_celkem","predikce_vzdelani_celkem",
                        "predikce_vek_okr_celkem","predikce_vzdelani_okr_celkem",
                        "aktualne_celkem")
...
vlada["pomer_predikce_vek_okr"] = vlada["predikce_vek_okr_adj"]/vlada["predikce_vek_okr_celkem"]
vlada["pomer_predikce_vzdelani_okr"] = vlada["predikce_vzdelani_okr_adj"]/vlada["predikce_vzdelani_okr_celkem"]
...
vysledky_rychlost["rozdil_vek_okr"] =  vysledky_rychlost$predikce_vek_okr-vysledky_rychlost$vysledek
vysledky_rychlost["rozdil_vzdelani_okr"] =  vysledky_rychlost$predikce_vzdelani_okr-vysledky_rychlost$vysledek
...
rychlost = aggregate(cbind(rozdil_strata,
                           rozdil_vek,rozdil_vzdelani,
                           rozdil_vek_okr,rozdil_vzdelani_okr,
                           rozdil_aktualne) ~ procent_secteno,vysledky_rychlost,  FUN=normF)
...
plot_list[["rychlost"]] <- ggplot(data=rychlost)+
  ...
  geom_line(aes(x=procent_secteno/100, y=rozdil_vzdelani_okr, color="Predikce vzdelani okrsky")) + 
  geom_line(aes(x=procent_secteno/100, y=rozdil_vek_okr, color="Predikce vek okrsky")) + 
...
vysledky_rychlost2[,2:15]=abs(vysledky_rychlost2[,2:15])

rychlost2 = aggregate(cbind(rozdil_strata,rozdil_vek,rozdil_vzdelani,
                            rozdil_vek_okr,rozdil_vzdelani_okr,rozdil_aktualne) ~ procent_secteno,vysledky_rychlost2,  FUN=mean)
...
pdf("obrazky/test_ran_VV_okrsky.pdf", encoding="ISOLatin2.enc",width=6.5,height = 5)
plot_list
dev.off()


save(vysledky_vse, vysledky_souboj, vetsina,vlada,rychlost,rychlost2, file = "vysledky/data_ran_VV_okrsky.RData")
