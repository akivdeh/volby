...
#Sys.setlocale(category = 'LC_CTYPE','en_US.UTF-8')
  ...
  ### Spustit predikci
  vysledky_vse = data.frame(strana=character(), aktualne=numeric(), predikce_strata=numeric(),predikce_strata14=numeric(),predikce_kraje=numeric(), procent_secteno=numeric())
  ...
  for (procent_secteno in seq(start, stop, by=5)) {
    ...
    #########
    #### Multinom 14 clusteru
    # Nacti
    #            load("/Users/admin/Desktop/reseni_0922/testovani/ciselnik17.RData")
    load("ciselniky/ciselnik14.RData")
    # Nacti
    #vstup <- read.table(paste0(paste0("/Users/User/Desktop/MFF/sfg_volby/reseni_0922/testovani/secteno_upravene/secteno", as.character(procent_secteno)), ".csv"), sep=",", header = T)
    ...
    names(vysledky) = c("strana", "predikce_strata14")
    
    vysledky_strata14 = vysledky
    ######## 
    ######## Kraje
    ...
    ####### Merge
    vysledky = merge(merge(vysledky_strata, vysledky_kraje, by="strana"),vysledky_strata14,by="strana")
...
for(strana in strany){
  ...
  plot <- ggplot(data=vysledky_plot[vysledky_plot$procent_secteno>=5, ])+
    geom_line(aes(x=procent_secteno/100, y=predikce_strata, color="Predikce strata (7)")) +
    geom_line(aes(x=procent_secteno/100, y=predikce_strata14, color="Predikce strata (14)")) + 
...
vysledky_souboj = vysledky_vse[vysledky_vse$strana=="kstrana_13", c("procent_secteno", "predikce_strata","predikce_strata14","predikce_kraje", "aktualne")]
names(vysledky_souboj) = c("procent_secteno", "predikce__strata_spolu","predikce_strata14_spolu","predikce_kraje_spolu", "aktualne_spolu")

vysledky_souboj = merge(vysledky_souboj, vysledky_vse[vysledky_vse$strana=="kstrana_20", c("procent_secteno", "predikce_strata","predikce_strata14","predikce_kraje", "aktualne")])
names(vysledky_souboj) = c("procent_secteno", "predikce_strata_spolu","predikce_strata14_spolu","predikce_kraje_spolu", "aktualne_spolu","predikce_strata_ano","predikce_strata14_ano","predikce_kraje_ano", "aktualne_ano")

vysledky_souboj["predikce_strata"] = vysledky_souboj["predikce_strata_ano"] - vysledky_souboj["predikce_strata_spolu"]
vysledky_souboj["predikce_strata14"] = vysledky_souboj["predikce_strata14_ano"] - vysledky_souboj["predikce_strata_spolu"]
...
vysledky_vetsina["predikce_strata14_adj"] = vysledky_vetsina$predikce_strata14*(vysledky_vetsina$predikce_strata14>=0.05)
vysledky_vetsina["predikce_kraje_adj"] = vysledky_vetsina$predikce_kraje*(vysledky_vetsina$predikce_kraje>=0.05)
vysledky_vetsina["aktualne_adj"] = vysledky_vetsina$aktualne*(vysledky_vetsina$aktualne>=0.05)

vetsina = aggregate(cbind(predikce_strata_adj,predikce_strata14_adj,predikce_kraje_adj, aktualne_adj) ~ koalice + procent_secteno,vysledky_vetsina,  sum)
vetsina_celkem = aggregate(cbind(predikce_strata_adj,predikce_strata14_adj,predikce_kraje_adj, aktualne_adj) ~ procent_secteno,vetsina,  sum)
names(vetsina_celkem) = c("procent_secteno", "predikce_strata_celkem","predikce_strata14_celkem","predikce_kraje_celkem", "aktualne_celkem")
...
vetsina["pomer_predikce_strata14"] = vetsina["predikce_strata14_adj"]/vetsina["predikce_strata14_celkem"]
...
plot_list[["vetsina"]] <- ggplot(data=vetsina[(vetsina$koalice==1) & (vetsina$procent_secteno>=5), ])+
  geom_line(aes(x=procent_secteno/100, y=pomer_predikce_strata, color="Predikce strata (7)")) + 
  geom_line(aes(x=procent_secteno/100, y=pomer_predikce_strata14, color="Predikce strata (14)")) + 
...
vysledky_vlada["predikce_strata14_adj"] = vysledky_vlada$predikce_strata14*(vysledky_vlada$predikce_strata14>=0.05)
vysledky_vlada["predikce_kraje_adj"] = vysledky_vlada$predikce_kraje*(vysledky_vlada$predikce_kraje>=0.05)
vysledky_vlada["aktualne_adj"] = vysledky_vlada$aktualne*(vysledky_vlada$aktualne>=0.05)

vlada = aggregate(cbind(predikce_strata_adj,predikce_strata14_adj,predikce_kraje_adj, aktualne_adj) ~ koalice + procent_secteno,vysledky_vlada,  sum)
vlada_celkem = aggregate(cbind(predikce_strata_adj,predikce_strata14_adj,predikce_kraje_adj, aktualne_adj) ~ procent_secteno,vlada,  sum)
names(vlada_celkem) = c("procent_secteno", "predikce_strata_celkem","predikce_strata14_celkem","predikce_kraje_celkem", "aktualne_celkem")
...
vlada["pomer_predikce_strata14"] = vlada["predikce_strata14_adj"]/vlada["predikce_strata14_celkem"]
...
pdf("obrazky/test_aposteriori_upravene_ran_14.pdf", encoding="ISOLatin2.enc")
plot_list
dev.off()


save(vysledky_vse, vysledky_souboj, vetsina,vlada, file = "vysledky/aposteriori_analyza_data_upravene_ran_22.RData")
