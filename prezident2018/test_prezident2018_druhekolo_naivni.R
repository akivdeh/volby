...
vstup_vse<-read.table("prezident2018/kolo_2.csv", sep=",", header = T)
old <- read.table("prezident2018/kolo_1.csv", sep=",", header=T);
load("prezident2018/vysledky_clustery.RData")
vysledky_clustery<-vysledky_clustery_volby
...
jmena<-c("Milos Zeman", "Jiri Drahos")
kandidati<-c("kandidat_7","kandidat_9")
...
temp_strany <- colnames(okrsky)[grepl('kandidat', colnames(okrsky))]
...
celkove_vysledky_procenta<-celkove_vysledky/sum(celkove_vysledky[1:9])
tabulka_kandidati$vysledky<-celkove_vysledky[tabulka_kandidati$kandidati]
tabulka_kandidati$vysledkyproc<-celkove_vysledky_procenta[tabulka_kandidati$kandidati]


clustery<-t(vysledky_clustery[,c("kandidat_7","kandidat_9")])
colnames(clustery)<-vysledky_clustery$cluster_name

tabulka_kandidati<-cbind(tabulka_kandidati,clustery)

clustery2<-clustery
clustery2[1,]<-clustery[1,]/apply(vysledky_clustery[,5:13],MARGIN=1,FUN=sum)
clustery2[2,]<-clustery[2,]/apply(vysledky_clustery[,5:13],MARGIN=1,FUN=sum)
colnames(clustery2)<-paste0(vysledky_clustery$cluster_name,"proc")

tabulka_kandidati<-cbind(tabulka_kandidati,clustery2)


##### Spustit predikci #####
vysledky_vse = data.frame(kandidat=character(), aktualne=numeric(),
                          predikce_strata=numeric(),
                          naivni_procenta=numeric(),
                          naivni_okrsky=numeric(),
                          naivni_okrsky_strata=numeric())
...
for (procent_secteno in seq(start, stop, by=5)) {
  ...
  print("procent secteno")
  print(procent_secteno)
  print("Neznámých: ")
  ...
  ####### Naivni procenta/okrsky   ####### 
  vysledky2<-colSums(vstup_posledni[,name_kandidat])
  procenta2<-vysledky2/sum(vysledky2)
  
  aktualni<-procenta2
  
  vysledky1_okrsky<-colSums(okrsky[okrsky$id %in% vstup_posledni$id,temp_strany])
  procenta1_okrsky<-vysledky1_okrsky/sum(vysledky1_okrsky[1:9])
  vysledky1_okrsky<-vysledky1_okrsky[tabulka_kandidati$kandidati]
  procenta1_okrsky<-procenta1_okrsky[tabulka_kandidati$kandidati]
  ...
  vysledky1_procenta<-colSums(okrsky1[,temp_strany])
  procenta1_procenta<-vysledky1_procenta/sum(vysledky1_procenta[1:9])
  vysledky1_procenta<-vysledky1_procenta[tabulka_kandidati$kandidati]
  procenta1_procenta<-procenta1_procenta[tabulka_kandidati$kandidati]
  # names(vysledky17_procenta)<-tabulka_strany$jmena
  # names(procenta17_procenta)<-tabulka_strany$jmena  
  
  vys_okrsky<-vysledky2/vysledky1_okrsky*tabulka_kandidati$vysledky
  odhadcelku<-sum(vys_okrsky)
  vys_okrsky2<-vys_okrsky/odhadcelku
  # vyspred_p<-vys/celkem17
  
  vys_procenta<-procenta2/procenta1_procenta*tabulka_kandidati$vysledkyproc
  ...
  vysledky<-data.frame(kandidat=names(aktualni),
                       predikce_raw=vys_okrsky2, predikce_raw2=vys_procenta)
  ...
  vysledky_naivni = vysledky[, c("kandidat", "predikce","predikce2")]
  names(vysledky_naivni) = c("kandidat", "naivni_okrsky","naivni_procenta")
  

  ####### Naivni okrsky stratifikace  ####### 

  ### clusterove vysledky druhe kolo
  vysledky_clustery2 = aggregate(. ~ cluster_name, data_melted[, c("cluster_name", name_kandidat)], sum)
  
  
  ### clusterove vysledky prvni kolo 
  old1<-old[1:round(procent_secteno*(length(old$cis_obec))/100),]
  old_posledni <- aggregate(poradi_zprac ~ cis_obec + cis_okrsek, old1, FUN = max)
  old_posledni <- merge(old1, old_posledni, by=c("cis_obec", "cis_okrsek", "poradi_zprac"), all.y=T)
  
  
  data <- merge(ciselnik_okrsky, old_posledni, by=c("cis_obec", "cis_okrsek","id"), all=T);
  indx_cluster <- grepl('comp', colnames(data))
  name_cluster <- colnames(data)[indx_cluster]
  indx_kandidat <- grepl('kandidat_', colnames(data))
  name_kandidat <- colnames(data)[indx_kandidat]
  ...
  # Agregace
  vysledky_old2 = aggregate(. ~ cluster_name, data_melted[, c("cluster_name", name_kandidat)], sum)
  vysledky_old2<-vysledky_old2[,c("cluster_name","kandidat_7","kandidat_9")]
  names(vysledky_old2)<-c("cluster_name","kandidat_7_1k","kandidat_9_1k")
  data_naivni<-merge(vysledky_clustery2,vysledky_old2,by="cluster_name")

  final<-t(tabulka_kandidati[,paste0("comp.",1:7)])
  colnames(final)<-c("kandidat_7_fin","kandidat_9_fin")
  data_naivni<-cbind(data_naivni,final)

  data_naivni$kandidat_7_pred<-data_naivni$kandidat_7_fin*data_naivni$kandidat_7/data_naivni$kandidat_7_1k 
  data_naivni$kandidat_9_pred<-data_naivni$kandidat_9_fin*data_naivni$kandidat_9/data_naivni$kandidat_9_1k 
  
  name_kandidat<-c("kandidat_7","kandidat_9")
  vysl<-c(sum(data_naivni$kandidat_7_pred),sum(data_naivni$kandidat_9_pred))
  #vysl<-vysl/sum(vysl)
  #pom = colSums(vysledky_clustery[, name_kandidat]*vysledky_clustery$vaha)
  vysledky = data.frame(vysl, row.names=name_kandidat)
  vysledky["predikce_raw"] = vysledky/sum(vysledky)
  vysledky["kandidat"] = rownames(vysledky)
  rownames(vysledky) = c()
  
  # Současný stav
  vysledky["aktualne_hlasu"] = colSums(data1[, name_kandidat])
  vysledky["aktualne"] = vysledky["aktualne_hlasu"]/sum(vysledky["aktualne_hlasu"])
  ...
  names(vysledky) = c("kandidat", "naivni_okrsky_strata")
  
  vysledky_naivni_strata = vysledky
  
  
  
  
  ####### Merge   ####### 
  df_list <- list(vysledky_strata, vysledky_naivni,vysledky_naivni_strata)      
...
for(k in 1:length(tabulka_kandidati$kandidati)){
  ...
  plot <- ggplot(data=vysledky_plot[vysledky_plot$procent_secteno>=5, ])+
    ...
    geom_line(aes(x=procent_secteno/100, y=naivni_okrsky, color="Predikce naivni (okrsky)")) + 
    geom_line(aes(x=procent_secteno/100, y=naivni_procenta, color="Predikce naivni (procenta)")) +
    geom_line(aes(x=procent_secteno/100, y=naivni_okrsky_strata, color="Predikce naivni (okrsky statifikovano)")) +
...
plot_list[["kandidat_9"]]
plot_list[["kandidat_7"]]
...
#indx_predikce <- colnames(vysledky_vse)[3:6]
name_predikce <- colnames(vysledky_vse)[3:6]
...
vysledky_souboj["naivni_okrsky"] = vysledky_souboj["naivni_okrsky Zeman"] - vysledky_souboj["naivni_okrsky Drahos"]
vysledky_souboj["naivni_procenta"] = vysledky_souboj["naivni_procenta Zeman"] - vysledky_souboj["naivni_procenta Drahos"]
vysledky_souboj["naivni_okrsky_strata"] = vysledky_souboj["naivni_okrsky_strata Zeman"] - vysledky_souboj["naivni_okrsky_strata Drahos"]
...
# 
# rychlost = aggregate(cbind(rozdil_strata,rozdil_kraje,rozdil_vek,rozdil_vzdelani,
#                            rozdil_vek_okr,rozdil_vzdelani_okr,rozdil_aktualne) ~ procent_secteno,vysledky_rychlost,  FUN=normF)
# 
...
# vysledky_rychlost2[,2:17]=abs(vysledky_rychlost2[,2:17])
# 
# rychlost2 = aggregate(cbind(rozdil_strata,rozdil_kraje,rozdil_vek,rozdil_vzdelani,
...
pdf("prezident2018/test_prez2018_all2k_naivni.pdf", encoding="ISOLatin2.enc",width=6.5,height = 5)
plot_list
dev.off()


save(vysledky_vse, vysledky_souboj, file = "prezident2018/data_prez2018_all2k_naivni.RData")
