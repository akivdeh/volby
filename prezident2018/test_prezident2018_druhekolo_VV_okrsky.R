...
vysledky_vse = data.frame(kandidat=character(), aktualne=numeric(),
                          predikce_strata=numeric(),predikce_kraje=numeric(),
                          predikce_vek=numeric(),predikce_vzdelani=numeric(),
                          predikce_vek_okr=numeric(),predikce_vzdelani_okr=numeric(),
                          procent_secteno=numeric())
...
for (procent_secteno in seq(start, stop, by=5)) {
  ...
  ### Join ciselnik
  data <- merge(ciselnik_vek_okr, vstup_posledni, by=c("cis_obec", "cis_okrsek","id"), all=T);
  indx_cluster <- grepl('comp', colnames(data))
  name_cluster <- colnames(data)[indx_cluster]
  indx_kandidat <- grepl('kandidat_', colnames(data))
  name_kandidat <- colnames(data)[indx_kandidat]
  ...
  vysledky = vysledky[, c("kandidat", "predikce")]
  names(vysledky) = c("kandidat", "predikce_vek_okr")
  ...
  ### Join ciselnik
  data <- merge(ciselnik_vzdelani_okr, vstup_posledni, by=c("cis_obec", "cis_okrsek","id"), all=T);
  indx_cluster <- grepl('comp', colnames(data))
  name_cluster <- colnames(data)[indx_cluster]
  indx_kandidat <- grepl('kandidat_', colnames(data))
  name_kandidat <- colnames(data)[indx_kandidat]
  ...
  vysledky = vysledky[, c("kandidat", "predikce")]
  names(vysledky) = c("kandidat", "predikce_vzdelani_okr")
  ...
  df_list <- list(vysledky_strata, vysledky_kraje,vysledky_vzdelani,
...
vysledky_souboj["predikce_vek_okr"] = vysledky_souboj["predikce_vek_okr Zeman"] - vysledky_souboj["predikce_vek_okr Drahos"]
vysledky_souboj["predikce_vzdelani_okr"] = vysledky_souboj["predikce_vzdelani_okr Zeman"] - vysledky_souboj["predikce_vzdelani_okr Drahos"]
...
pdf("prezident2018/test_prez2018_all2k_VV_okrsky.pdf", encoding="ISOLatin2.enc",width=6.5,height = 5)
plot_list
dev.off()


save(vysledky_vse, vysledky_souboj,rychlost,rychlost2, file = "prezident2018/data_prez2018_all2k_VVokrsky.RData")
